...
class TokenChecker(object):
  ...
  def requestAvatarId(self, credentials):
    ...
    try:
      ...
      token_roles = [role['name'] for role in token_info['token']['roles']]
      for role in self.enforce_roles:
        if not role in token_roles:
          raise InvalidUserToken
...
class ResourceShield(object):
  ...
  def __init__(self, config_file_name=None, config_override=None):
    ...
    there instead of from what's specified by TWISTED_KEYSTONE_CONFIG env var.
    You can override options by passing a dict via config_override.
    """
  ...
  def _getConfig(self, config_file_name, config_override):
    config_file_name = config_file_name or os.environ.get('TWISTED_KEYSTONE_CONFIG')
    if config_file_name:
      ...
      for option, value in parser.items('config'):
        try:
          # We need typed values (not strs). Try to parse them with json.
  ...
  def protectResource(self, resource, delay_auth_decision=False,
    ...
    If delay_auth_decision is False, unauthorized requests are replied with an
    HTTP 401 error response. Otherwise, all requests will be allowed to pass
    delegating the final auth decision to the service itself. Authorized
    resources are flagged by setting the keystone_authorized field to True.
    Also, the retrieved token info is stored in the keystone_token_info field.
    All roles passed in enforce_roles will be checked against the token. If
    any of them is missing, the request will not be authorized.
    """
    p = portal.Portal(ProtectedRealm(resource),
        [TokenChecker(self.auth_protocol, delay_auth_decision, enforce_roles)])
